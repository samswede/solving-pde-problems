...
function solve_adv_disp(
    v = 0.05,
    C_in = 1.0,
    Daxial = 1e-3,
    L = 1.0,
    tf = 10.0,
    solver_name = "Tsit5"  # Default solver name
)
...
function solve_adv_disp_pulse(params::PyDict)
    ...
    C_in_C(t) = amplitude * (tanh(100 * (t - start)) - tanh(100 * (t - end_time)))
...
function solve_adv_disp_wave(params::PyDict)
...
function solve_adv_disp_all(params::PyDict)
    ...
    C_in_A(t) = sin(t) + 1.1  # Ensure it's greater than 0
    C_in_B(t) = 0.5 * (tanh(100 * (t - 0.5)) - tanh(100 * (t - 1.5)))
    C_in_C(t) = 0.5
