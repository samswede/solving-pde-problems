...
class AdvDispParams(BaseModel):
    ...
    grid_size: int = 200  # Changed from num_elements to grid_size
...
def run(config):
    ...
    with mlflow.start_run():
        ...
        solution_array_A_julia, solution_array_B_julia, solution_array_C_julia = jl.solve_adv_disp(params.model_dump())
        ...
        print(f"solve_adv_disp_3 completed in {end_solution_time - start_time:.2f} seconds.")
...
def single_run():
    config = {
        ...
        "experiment_id": "adv_disp_experiment",
        "tags": {
            "problem": "adv_disp",
            "dimension": "1D",
            "num_components": 3,
            "num_phases": 1,
            "run_type": "single_run"  # Added run_type tag
        }
    }
    run(config)
...
def identify_best_method():
    ...
    optuna_config = {
        ...
        "run_name_prefix": "adv_disp",
        "pde_package": "julia",
        "parent_run": "adv_disp_parent"
    }
